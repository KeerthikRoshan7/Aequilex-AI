...
const createMockClient = () => {
  const mockStorage = { users: [], workspaces: [], chats: [], spaces: [] };
  return {
    from: (table) => {
      ...
      const chain = {
        ...
        eq: (k, v) => { queryResult = queryResult.filter(item => item[k] === v); return chain; },
        order: () => chain,
        single: async () => ({ data: queryResult[0] || null, error: queryResult[0] ? null : new Error("Invalid token or key.") }),
        then: (cb) => cb({ data: queryResult, error: null })
      };
      return chain;
    }
  };
...
const convertToWavBase64 = async (webmBlob) => {
...
export default function App() {
  ...
  const [workspaces, setWorkspaces] = useState([{ id: 0, name: 'General Workspace' }]);
  const [currentWorkspace, setCurrentWorkspace] = useState({ id: 0, name: 'General Workspace' });
...
// 1. RESEARCH CORE
// ==========================================
function ResearchCore({ user, currentWorkspace }) {
  ...
  const archiveMessage = async (content) => {
  ...
  const handleSend = async (e) => {
    ...
    let parts = [{ text: `User Query: ${queryText}` }];
    if (attachedFile) {
...
function TranslateDesk({ user }) {
  ...
  const handleTranslate = async () => {
    ...
    let parts = [{ text: `Translate the following legal text accurately to ${targetLang}. Preserve Latin maxims in brackets. Text: ${source}` }];
    if (attachedFile) parts.push({ inlineData: { mimeType: attachedFile.mimeType, data: attachedFile.data } });
...
// 4. KNOWLEDGE VAULT
// ==========================================
function KnowledgeVault({ user, currentWorkspace }) {
...
// NAVIGATION SIDEBAR
// ==========================================
function Sidebar({ user, onLogout, activeModule, setActiveModule, workspaces, currentWorkspace, setCurrentWorkspace, refreshWorkspaces }) {
    ...
    <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center font-inter text-[#E2E8F0]">
      ...
      {IS_MOCK && (
        <div className="absolute top-0 w-full bg-red-900/50 text-red-300 py-2 text-center text-xs font-bold uppercase tracking-widest border-b border-red-500/50">
          ⚠️ PREVIEW MODE ACTIVE: Database is mocked. To connect to real Supabase, change IS_MOCK to false in page.jsx!
        </div>
      )}
...
// ==========================================
function AequilexText({ className = "", animate = false, size = "large" }) {
...
// ==========================================
function LogoSVG({ className, animate = false }) {
  return (
